# TSAuto 项目技术设计文档 (AI协作优化版)

**项目代号：** TSAuto  
**文档版本：** v2.0 (AI协作优化版)  
**最后更新：** 2025-01-07  
**核心设计原则：** 界面与逻辑分离，支持多UI框架接入
...
3. [目录结构树](#3-目录结构树)
4. [技术栈详解](#4-技术栈详解)
5. [类型定义系统 (SSOT)](#5-类型定义系统-ssot)
6. [核心战斗系统](#6-核心战斗系统)
7. [状态同步与UI桥接层](#7-状态同步与ui桥接层)
8. [单元测试驱动开发](#8-单元测试驱动开发)
9. [插件系统架构](#9-插件系统架构)
10. [伤害公式与边界处理](#10-伤害公式与边界处理)
11. [AI助手协作指南](#11-ai助手协作指南)
12. [部署与跨平台策略](#12-部署与跨平台策略)
...
### 1.1 项目定位
开发一个**以复杂2D回合制战斗为核心**的跨平台刷子游戏，技术架构支持未来无缝接入不同的UI框架。

### 1.2 核心需求
- **跨平台覆盖：** Windows, macOS, Linux, iOS, Android
- **双运行模式：** 本地模式 (IndexedDB) + 联网模式 (PostgreSQL + WebSocket)
- **界面逻辑分离：** 核心逻辑完全独立于UI框架，支持React、Vue、Angular等
- **复杂战斗引擎：** 对标《阴阳师》的深度战斗机制
- **动态插件系统：** 角色、技能、装备等热插拔
- **术语可配置：** 全局术语替换，规避版权风险
...
│   (React/Vue/etc)   │    │   (React/Vue/etc)   │    │   (React/Vue/etc)   │
└─────────┬───────────┘    └─────────┬───────────┘    └─────────┬───────────┘
          │                          │                          │
          └──────────────┬─────────────────┬──────────────────┘
                         │                 │
...
│   │   │   └── gameConfig.ts      # 游戏配置
│   │   ├── services/              # 业务服务
│   │   │   ├── CharacterStatsService.ts # 角色属性计算
│   │   │   ├── EquipmentService.ts      # 装备系统
│   │   │   ├── FormulaService.ts        # 公式服务
│   │   │   └── RandomService.ts         # 随机数服务
│   │   ├── types/                 # 🔥 类型定义 (SSOT)
│   │   │   ├── definitions.ts    # 核心接口定义
│   │   │   ├── battle.ts         # 战斗状态类型
│   │   │   └── plugin.ts         # 插件类型
│   │   ├── plugin-system/         # 插件系统
...
│   │   └── utils/                # 工具函数
│   │
│   ├── ui-bridge/                 # 🔥 UI桥接层 (可适配多框架)
│   │   ├── react/                # React适配器
│   │   │   ├── useBattleStore.ts # Zustand状态桥接
│   │   │   ├── BattleAdapter.ts  # 战斗状态适配器
│   │   │   └── hooks/            # React专用hooks
│   │   ├── vue/                  # Vue适配器 (未来扩展)
│   │   ├── angular/              # Angular适配器 (未来扩展)
│   │   └── common/               # 通用桥接接口
│   │
│   ├── ui-react/                  # 🔥 React UI层 (可替换)
│   │   ├── components/           # React组件
...
### 4.2 UI桥接层
- **React适配：** Zustand + React 18 + TailwindCSS
- **Vue适配：** Pinia + Vue 3 (规划中)
- **通用接口：** 标准化状态和事件接口
...
**⚠️ 关键约束：** 所有类型定义必须集中在 `src/core/types/` 目录，禁止在其他文件中定义重复类型。
...
**开发约束：**
1. **禁止重复定义：** 相同概念的接口不得在不同文件中重复定义
2. **禁止扩展未定义属性：** 不得为接口添加文档中未列出的属性
3. **严格类型检查：** 所有代码必须通过TypeScript严格模式检查
...
export class TurnManager {
   ...
   * 修改行动条 (推拉条)
   * @param target 目标角色
   * @param percentage 推拉条比例 (-1.0 到 1.0)
   */
...
**核心约束：**
1. **UI组件禁止直接修改BattleState**
2. **所有状态变更必须通过BattleEngine**
3. **状态更新使用不可变数据模式**
4. **UI桥接层适配不同UI框架**
...
**`src/ui-bridge/vue/BattleAdapter.ts` (未来实现)**
...
describe('TurnManager', () => {
  ...
  beforeEach(() => {
    // 创建测试角色实例
    mockCharacters = [
      createMockCharacter('char_A', 100), // 速度100
      createMockCharacter('char_B', 200)  // 速度200 (全场一速)
    ];
  ...
  describe('推拉条机制', () => {
    it('应该正确执行拉条30%', () => {
      ...
      turnManager.modifyActionBar(target, 0.3); // 拉条30%
      
      const expectedChange = 200 * 0.3; // 全场一速 × 30%
    ...
    it('不应该让行动条位置低于0', () => {
      ...
      turnManager.modifyActionBar(target, -0.2); // 推条20%
...
describe('DamageCalculator', () => {
  describe('基础伤害计算', () => {
    ...
    it('应该处理防御为0的情况', () => {
      ...
      // 防御为0时应该造成更高伤害
      expect(damage).toBeGreaterThan(500); // 保守估计
    });
  });
  ...
  describe('防御穿透测试', () => {
    it('应该正确处理无视防御百分比', () => {
      ...
      // 50%无视防御应该比0%无视防御造成更多伤害
      expect(damage).toBeGreaterThan(800); // 保守估计
    });
    ...
    it('应该处理防御为负数的情况', () => {
      ...
      expect(damage).toBeGreaterThan(1000); // 防御为负应该增伤
    });
  });
...
/**
 * 插件加载器
 * 使用 Vite 的 import.meta.glob 动态加载插件
 */
...
**角色插件示例：** `src/plugins/characters/character_1001.ts`
...
**技能插件示例：** `src/plugins/skills/skill_1001.ts`
...
export const skill_1001: SkillDefinition = {
  ...
  // 技能描述 (用于UI显示)
  description: "对敌方目标造成120%攻击的伤害，并提升自身行动条20%，获得1层风盾效果。"
};
```

**装备插件示例：** `src/plugins/equipment/equip_2001.ts`
...
**状态插件示例：** `src/plugins/statuses/status_3001.ts`
...
export class DamageCalculator {
  ...
  static calculateDamage(
    ...
    // 1. 基础攻击力计算
    const baseAtk = this.calculateBaseAttack(caster);
    
    // 2. 攻击增益计算
    const atkBonus = this.calculateAttackBonus(caster);
   ...
   * 计算攻击增益 (百分比)
   */
  private static calculateAttackBonus(caster: CharacterInstance): number {
    let bonus = 0;
    
    // 基础攻击加成
    bonus += caster.currentStats.ATK_P || 0;
    
    // Buff攻击加成
    bonus += this.getBuffBonus(caster, 'ATK_P');
...
```
你是一名资深的游戏引擎架构师，精通TypeScript和React性能优化。
...
术语检查：
每次输出代码前，自我检查是否使用了硬编码字符串。
如有发现，立即替换为TERMS引用。

禁止假设：
如果文档中未定义某个机制，先询问用户确认，不要自行发挥。
```
...
1. **类型定义冲突** ❌
   - 错误：在多个文件定义相同接口
   - 正确：统一在 `src/core/types/` 定义

2. **硬编码术语** ❌
   - 错误：`"鬼火"` 直接写在组件中
   - 正确：使用 `TERMS.BATTLE_RESOURCE`

3. **UI直接修改状态** ❌
   - 错误：React组件直接修改BattleState
   - 正确：通过BattleEngine方法
...
│                     跨平台架构                                │
├─────────────────────────────────────────────────────────────┤
│  桌面应用 (Tauri)  │  移动应用 (Capacitor)  │  Web应用 (Vite)  │
├─────────────────────────────────────────────────────────────┤
│                    UI框架层 (可替换)                          │
│  React Components  │  Vue Components     │  Other Frameworks │
├─────────────────────────────────────────────────────────────┤
│                 UI桥接层 (标准化接口)                         │
│    ReactAdapter    │    VueAdapter       │   Common Bridge   │
├─────────────────────────────────────────────────────────────┤
│                   核心逻辑层 (纯TS)                          │
│  BattleEngine  │  TurnManager  │  DamageCalculator  │  Plugins │
├─────────────────────────────────────────────────────────────┤
│                 跨平台封装层                                 │
│      Tauri Rust Core      │        Capacitor Native        │
└─────────────────────────────────────────────────────────────┘
```
...
```json
{
  "scripts": {
    "dev": "vite",                           // Web开发模式
    "build:web": "vite build",               // Web构建
    "build:tauri": "tauri build",            // 桌面应用构建
    "build:capacitor": "capacitor build",    // 移动应用构建
    "test": "vitest",                        // 运行测试
...
export default defineConfig({
  ...
  build: {
    lib: {
      entry: 'src/core/index.ts',           // 核心逻辑独立打包
      formats: ['es'],                      // ES模块格式
      fileName: 'core'                      // 文件名
    },
    rollupOptions: {
